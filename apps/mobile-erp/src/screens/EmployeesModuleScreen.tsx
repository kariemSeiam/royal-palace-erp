...
import { getEmployees, getDepartments, getFactories } from "../features/erp/api";
import { erpColors } from "../theme/tokens";

export default function EmployeesModuleScreen() {
  const results = useQueries({
    queries: [
      { queryKey: ["mobile-erp-employees"], queryFn: getEmployees, retry: false },
      { queryKey: ["mobile-erp-employees-departments"], queryFn: getDepartments, retry: false },
      { queryKey: ["mobile-erp-employees-factories"], queryFn: getFactories, retry: false }
    ]
  });

  const [employeesQuery, departmentsQuery, factoriesQuery] = results;

  if (results.some((item) => item.isLoading)) {
    return (
      <SafeAreaView edges={["left", "right"]} style={screen}>
        <View style={centered}>
          <Text style={loadingText}>جاري تحميل الموظفين...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    return (
      <SafeAreaView edges={["left", "right"]} style={screen}>
        <View style={errorWrap}>
          <View style={box}>
            <Text style={boxTitle}>تعذر تحميل الموظفين</Text>
  ...
  return (
    <SafeAreaView edges={["left", "right"]} style={screen}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 140 }}>
        <View style={hero}>
          <Text style={heroTop}>EMPLOYEES</Text>
          <Text style={heroTitle}>الموظفون</Text>
          <Text style={heroBody}>عرض نفس بيانات الموظفين الحالية مع القسم والمصنع والحالة الوظيفية.</Text>
        </View>

        <View style={row}>
          <StatCard label="إجمالي الموظفين" value={employees.length} />
          <StatCard label="النشطون" value={active} />
        </View>

        <View style={box}>
          <Text style={boxTitle}>قائمة الموظفين</Text>
          <View style={{ marginTop: 12, gap: 10 }}>
            {employees.length ? employees.slice(0, 40).map((item) => (
              <View key={String(item.id)} style={itemCard}>
                <Text style={itemTitle}>{`${item.first_name || ""} ${item.last_name || ""}`.trim() || `Employee #${item.id}`}</Text>
                <Text style={itemBody}>الكود: {item.employee_code || "-"}</Text>
                <Text style={itemBody}>المصنع: {factoryMap[item.factory_id] || `#${item.factory_id}`}</Text>
                <Text style={itemBody}>القسم: {departmentMap[item.department_id] || `#${item.department_id}`}</Text>
                <Text style={itemBody}>المنصب: {item.job_title || "-"}</Text>
                <Text style={itemBody}>الحالة الوظيفية: {item.employment_status || "-"}</Text>
                <Text style={itemBody}>الهاتف: {item.phone || "-"}</Text>
                <Text style={itemBody}>البريد: {item.email || "-"}</Text>
            ...
            )) : <Text style={boxBody}>لا توجد بيانات موظفين.</Text>}
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
