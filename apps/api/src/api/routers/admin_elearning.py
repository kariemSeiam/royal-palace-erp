...
router = APIRouter(prefix="/admin/elearning", tags=["admin-elearning"])
...
async def list_items(db: AsyncSession = Depends(get_db), current_user = Depends(get_current_user_and_role)):
    result = await db.execute(text("SELECT * FROM elearning_courses ORDER BY id DESC"))
    rows = result.all()
    def serialize(row):
        d = dict(row._mapping)
        for k, v in d.items():
...
async def create_item(data: dict, db: AsyncSession = Depends(get_db), current_user = Depends(get_current_user_and_role)):
    ...
    await db.execute(text(f"INSERT INTO elearning_courses ({columns}) VALUES ({placeholders})"), payload)
    await db.commit()
    return {"ok": True}
...
async def update_item(item_id: int, data: dict, db: AsyncSession = Depends(get_db), current_user = Depends(get_current_user_and_role)):
    ...
    payload["id"] = item_id
    await db.execute(text(f"UPDATE elearning_courses SET {set_clause} WHERE id = :id"), payload)
    await db.commit()
    return {"ok": True}

@router.delete("/{item_id}")
async def delete_item(item_id: int, db: AsyncSession = Depends(get_db), current_user = Depends(get_current_user_and_role)):
    await db.execute(text(f"DELETE FROM elearning_courses WHERE id = :id"), {"id": item_id})
    await db.commit()
    return {"ok": True}
