...
const API_URL = "https://api.royalpalace-group.com/api/v1/admin/timesheets/";

export default function TimesheetsPage() {
  const { user, ready } = useAdminAuth("timesheets");
  ...
  function csvExport() { exportTableCsv("timesheets.csv", ["الاسم","الوصف"], filtered.map(i => [i.name||"", i.description||""])); }
  function pdfExport() { exportTablePdf("تقرير timesheets", "تقرير", [{label:"العدد",value:items.length}], ["الاسم","الوصف"], filtered.map(i => [i.name||"", i.description||""])); }

  return (
    <main className="erp-shell" dir="rtl">
      <Sidebar user={user} />
      <section className="erp-main">
        <section className="erp-hero"><div><div className="erp-hero-pill">timesheets</div><h2>timesheets</h2><p>إدارة timesheets</p></div><div><button className="erp-btn-secondary" onClick={()=>setShowForm(!showForm)}>{showForm ? "إخفاء النموذج" : "فتح النموذج"}</button><button className="erp-btn-primary" onClick={()=>{resetForm();setShowForm(true);}}>إضافة جديد</button></div></section>
