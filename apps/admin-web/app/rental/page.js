...
const API_URL = "https://api.royalpalace-group.com/api/v1/admin/rental/";

export default function RentalPage() {
  const { user, ready } = useAdminAuth("rental");
  ...
  function csvExport() { exportTableCsv("rental.csv", ["الاسم","الوصف"], filtered.map(i => [i.name||"", i.description||""])); }
  function pdfExport() { exportTablePdf("تقرير rental", "تقرير", [{label:"العدد",value:items.length}], ["الاسم","الوصف"], filtered.map(i => [i.name||"", i.description||""])); }

  return (
    <main className="erp-shell" dir="rtl">
      <Sidebar user={user} />
      <section className="erp-main">
        <section className="erp-hero"><div><div className="erp-hero-pill">rental</div><h2>rental</h2><p>إدارة rental</p></div><div><button className="erp-btn-secondary" onClick={()=>setShowForm(!showForm)}>{showForm ? "إخفاء النموذج" : "فتح النموذج"}</button><button className="erp-btn-primary" onClick={()=>{resetForm();setShowForm(true);}}>إضافة جديد</button></div></section>
