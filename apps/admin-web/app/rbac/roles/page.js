...
export default function RolesPage() {
  ...
  async function loadAll() {
    const res = await fetch("https://api.royalpalace-group.com/api/v1/admin/users/roles", {
      headers: authHeaders(),
      cache: "no-store",
    });
    const data = await res.json().catch(() => []);
    if (!res.ok) throw new Error(data.detail || "فشل تحميل الأدوار");
  ...
  if (!ready || !user) {
    return (
      <main className="loading-shell">
        <div className="loading-card">جارٍ تحميل الأدوار...</div>
      </main>
    );
  }
          ...
          <div className="erp-section-head">
            <div style={{ textAlign: "right" }}>
              ...
              <p style={{ margin: "6px 0 0", color: "var(--rp-text-muted)" }}>
                عرض الأدوار الحالية وعدد المستخدمين المرتبطين بها.
              </p>
            </div>
            <input
            ...
            <table className="erp-table">
              ...
              <tbody>
                {filteredItems.length === 0 ? (
                  <tr>
                    <td colSpan="4">
                      {items.length === 0 ? "لا توجد أدوار حالياً." : "لا توجد نتائج مطابقة."}
                    </td>
                  </tr>
                ) : (
