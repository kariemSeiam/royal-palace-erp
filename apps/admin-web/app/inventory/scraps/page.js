...
export default function StockScrapsPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setMessage("تم تسجيل الهالك");
      setForm({ factory_id:"", warehouse_id:"", product_id:"", lot_id:"", quantity:"", uom_id:"", scrap_reason:"", notes:"" });
      loadData();
    } catch (err) { setMessage(err.message); } finally { setSubmitting(false); }
  }

  async function markDone(id) {
    if (!confirm("تأكيد الهالك؟")) return;
    const res = await fetch(`${API_URL}/${id}/done`, { method:"PUT", headers:authHeaders() });
    if (!res.ok) { setMessage("فشل التأكيد"); return; }
    loadData();
  }
    ...
    <main className="erp-shell" dir="rtl"><Sidebar user={user} /><section className="erp-main">
      ...
      <div className="erp-section-card" style={{ marginBottom:18 }}>
        ...
        <form className="erp-form-grid" onSubmit={handleSubmit}>
          ...
          <select className="erp-input" value={form.warehouse_id} onChange={e => setForm({ ...form, warehouse_id:e.target.value })} required><option value="">اختر المخزن</option>{warehouses.map(w=><option key={w.id} value={w.id}>{w.name} ({w.code})</option>)}</select>
          ...
          <select className="erp-input" value={form.lot_id} onChange={e => setForm({ ...form, lot_id:e.target.value })}><option value="">اختر التشغيلة (اختياري)</option>{lots.filter(l=>!form.product_id||l.product_id===Number(form.product_id)).map(l=><option key={l.id} value={l.id}>{l.lot_number} ({l.quantity})</option>)}</select>
          ...
          <select className="erp-input" value={form.uom_id} onChange={e => setForm({ ...form, uom_id:e.target.value })}><option value="">وحدة القياس</option>{uoms.map(u=><option key={u.id} value={u.id}>{u.name}</option>)}</select>
          <select className="erp-input" value={form.scrap_reason} onChange={e => setForm({ ...form, scrap_reason:e.target.value })}><option value="">سبب الهالك</option><option value="تالف">تالف</option><option value="منتهي الصلاحية">منتهي الصلاحية</option><option value="إنتاج معيب">إنتاج معيب</option><option value="مفقود">مفقود</option></select>
          <textarea className="erp-input" rows="2" placeholder="ملاحظات" value={form.notes} onChange={e => setForm({ ...form, notes:e.target.value })} />
          <div className="erp-form-actions"><button className="erp-btn-primary" type="submit" disabled={submitting}>{submitting?"جارٍ التسجيل...":"تسجيل الهالك"}</button></div>
        </form>
      </div>
