...
const API_URL = "https://api.royalpalace-group.com/api/v1/admin/survey/";

export default function SurveyPage() {
  const { user, ready } = useAdminAuth("survey");
  ...
  function csvExport() { exportTableCsv("survey.csv", ["الاسم","الوصف"], filtered.map(i => [i.name||"", i.description||""])); }
  function pdfExport() { exportTablePdf("تقرير survey", "تقرير", [{label:"العدد",value:items.length}], ["الاسم","الوصف"], filtered.map(i => [i.name||"", i.description||""])); }

  return (
    <main className="erp-shell" dir="rtl">
      <Sidebar user={user} />
      <section className="erp-main">
        <section className="erp-hero"><div><div className="erp-hero-pill">survey</div><h2>survey</h2><p>إدارة survey</p></div><div><button className="erp-btn-secondary" onClick={()=>setShowForm(!showForm)}>{showForm ? "إخفاء النموذج" : "فتح النموذج"}</button><button className="erp-btn-primary" onClick={()=>{resetForm();setShowForm(true);}}>إضافة جديد</button></div></section>
