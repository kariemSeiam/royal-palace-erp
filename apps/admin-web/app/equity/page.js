...
const API_URL = "https://api.royalpalace-group.com/api/v1/admin/equity/";

export default function EquityPage() {
  const { user, ready } = useAdminAuth("equity");
  ...
  function csvExport() { exportTableCsv("equity.csv", ["الاسم","الوصف"], filtered.map(i => [i.name||"", i.description||""])); }
  function pdfExport() { exportTablePdf("تقرير equity", "تقرير", [{label:"العدد",value:items.length}], ["الاسم","الوصف"], filtered.map(i => [i.name||"", i.description||""])); }

  return (
    <main className="erp-shell" dir="rtl">
      <Sidebar user={user} />
      <section className="erp-main">
        <section className="erp-hero"><div><div className="erp-hero-pill">equity</div><h2>equity</h2><p>إدارة equity</p></div><div><button className="erp-btn-secondary" onClick={()=>setShowForm(!showForm)}>{showForm ? "إخفاء النموذج" : "فتح النموذج"}</button><button className="erp-btn-primary" onClick={()=>{resetForm();setShowForm(true);}}>إضافة جديد</button></div></section>
